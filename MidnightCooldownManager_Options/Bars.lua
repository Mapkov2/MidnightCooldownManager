...
StaticPopupDialogs["MidnightCooldownManager_CONFIRM_DELETE_BAR_GROUP"] = {
    text = "",
    button1 = YES,
    button2 = NO,
    OnAccept = function()
        local fn = StaticPopupDialogs["MidnightCooldownManager_CONFIRM_DELETE_BAR_GROUP"]._pendingDelete
        if fn then fn() end
    end,
    timeout = 0,
    whileDead = true,
...
local function CreateBarsTab(page)
    ...
    local function GetViewerSpellListForSpec(specID)
        ...
        local function AddByCooldownID(cdID, info)
            ...
            info = info or Shared.GetCooldownInfoByID(cdID)
            if not info then return end
            if not Shared.ShouldShowCooldownInfo(info) then return end
        ...
        Shared.ForEachCooldownViewerInfoByKind("aura", AddByCooldownID)

        API:ForEachActiveFrame({ CDM_C.VIEWERS.BUFF_BAR }, function(frame)
            AddByCooldownID(Shared.GetFrameCooldownID(frame))
        end)
    ...
    local function GetUntrackedViewerSpellListForCurrentSpec()
        local activeCdIDSet = {}
        API:ForEachActiveFrame({ CDM_C.VIEWERS.BUFF_BAR }, function(frame)
            local cooldownID = Shared.GetFrameCooldownID(frame)
            if cooldownID then activeCdIDSet[cooldownID] = true end
        end)
    ...
    local function BuildActiveBarSpellSet()
        local set = {}
        API:ForEachActiveFrame({ CDM_C.VIEWERS.BUFF_BAR }, function(frame)
            Shared.MarkFrameSpellCandidates(set, frame)
    ...
    local function GetUngroupedBarSpells()
        ...
        API:ForEachActiveFrame({ CDM_C.VIEWERS.BUFF_BAR }, function(frame)
            ...
            local displayID = Shared.GetFrameDisplaySpellID(frame)
    ...
    local leftScroll = CreateFrame("ScrollFrame", "MidnightCDM_BarGroupsLeftScroll", page, "ScrollFrameTemplate")
    ...
    signalText:SetText(L["Missing bars must be enabled in Blizzard Cooldown Settings under Tracked Bars for this spec. If a bar is under Not Displayed, MCDM cannot add it."])
    ...
    local function CreateRenderHelpers(rc)
        ...
        function h.SectionHeader(text)
            ...
            firstSection = false
            local fs = rc:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font18")
        ...
        function h.Label(text)
            local fs = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
            fs:SetText(text)
            fs:SetPoint("TOPLEFT", 0, yOff)
            yOff = yOff - 22
            return fs
        ...
        function h.Dropdown(width, defaultLabel)
            local dd = RegisterRightPanelDropdown(UI.CreateDropdown(rc))
    ...
    local function RenderSpellOverrideSettings(rc, gd, groupIndex, spellID)
        ...
        local barColorLabel = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
        ...
        local clearBtn = UI.CreateActionButton(rc, L["Clear"], 80, 22)
        ...
        local bgColorLabel = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
        ...
        local bgClearBtn = UI.CreateActionButton(rc, L["Clear"], 80, 22)
        ...
        local nameBox = UI.CreateModernEditBox(rc, 300, 22)
        ...
        h.AddYOff(-26)
        local hint = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font12")
    ...
    local function ShowSpellPickerPanel(groupIndex)
        ...
        local pickerRc = Shared.RenderSpellPicker({
            ...
            helpText = L["Missing bars must be enabled in Blizzard Cooldown Settings under Tracked Bars for this spec. If a bar is under Not Displayed, MCDM cannot add it."],
            helpButtonText = L["Open Settings"],
            onHelpButton = OpenBlizzardCooldownSettings,
            spells = spells,
            emptyText = currentSpecID == playerSpecID
                and L["No loaded bar entries are available. Open Blizzard Settings and move missing entries from Not Displayed to Tracked Bars."]
                or L["All loaded bar entries are assigned to groups. Missing bars must be enabled in Blizzard Settings first."],
    ...
    local function ConfigureHeaderRow(widget, groupIndex, groupData, isExpanded, isSelected, yOff)
        ...
        widget.deleteBtn:SetScript("OnClick", function()
            StaticPopupDialogs["MidnightCooldownManager_CONFIRM_DELETE_BAR_GROUP"].text = string.format(
                L["Delete bar group '%s'?"],
                groupData.name or ("Group " .. groupIndex))
            StaticPopupDialogs["MidnightCooldownManager_CONFIRM_DELETE_BAR_GROUP"]._pendingDelete = function()
            ...
            StaticPopup_Show("MidnightCooldownManager_CONFIRM_DELETE_BAR_GROUP")
        end)
    end
    ...
    local function ConfigureSpellRow(widget, parent, sid, groupIndex, yOff, isSelected, spellIndex, spellCount, tooltipOverrideMap, isActive)
        ...
        local hasMoveButtons = groupIndex and spellIndex and spellCount
    ...
    RefreshAll = function()
        ...
        if currentSpecID == playerSpecID then
            tooltipOverrideMap = {}
            Shared.ForEachCooldownViewerInfoByKind("bar", function(_, info)
        ...
        if not btnRefs.ungroupedSettings then
            local ugBtn = UI.CreateActionButton(leftChild, L["Settings"], 90, 22)
        ...
        if not btnRefs.group then
            local addGroupBtn = UI.CreateActionButton(page, L["Add Group"], 110, 22)
        ...
        if not btnRefs.spell then
            local addSpellBtn = UI.CreateActionButton(page, L["Add Spell"], 110, 22)
