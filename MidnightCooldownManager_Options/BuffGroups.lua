...
local function SaveVisualRefresh()
...
StaticPopupDialogs["MidnightCooldownManager_CONFIRM_DELETE_GROUP"] = {
    text = "",
    button1 = YES,
    button2 = NO,
    OnAccept = function()
        local fn = StaticPopupDialogs["MidnightCooldownManager_CONFIRM_DELETE_GROUP"]._pendingDelete
        if fn then fn() end
    end,
    timeout = 0,
    whileDead = true,
...
local function CreateBuffGroupsTab(page)
    ...
    local QueueWorldPreviewRefresh

    local function SaveAndRefresh()
        SaveVisualRefresh()
        if QueueWorldPreviewRefresh then
            QueueWorldPreviewRefresh()
        end
    end
    ...
    local function OpenBlizzardCooldownSettings()
        if API.IsCooldownViewerSettingsPanelVisible and API:IsCooldownViewerSettingsPanelVisible() then
            return
        end
    ...
    local worldPreviewTimer
    local function HideWorldPreview()
        if worldPreviewTimer then
            worldPreviewTimer:Cancel()
            worldPreviewTimer = nil
        end
        if API.HideBuffGroupSelectionPreview then
            API:HideBuffGroupSelectionPreview()
        end
    end

    local function RefreshWorldPreview()
        if not page:IsShown() or currentSpecID ~= playerSpecID then
            HideWorldPreview()
            return
        end

        if selectedSpellID and selectedSpellGroupIndex and API.ShowBuffGroupSelectionPreview then
            API:ShowBuffGroupSelectionPreview(selectedSpellGroupIndex, selectedSpellID, currentSpecID)
        elseif selectedGroupIndex and API.ShowBuffGroupSelectionPreview then
            API:ShowBuffGroupSelectionPreview(selectedGroupIndex, nil, currentSpecID)
        else
            HideWorldPreview()
        end
    end

    QueueWorldPreviewRefresh = function()
        if worldPreviewTimer then
            worldPreviewTimer:Cancel()
        end
        worldPreviewTimer = C_Timer.NewTimer(0, function()
            worldPreviewTimer = nil
            RefreshWorldPreview()
        end)
    end
    ...
    local function GetUngroupedBuffSpells()
        ...
        API:ForEachActiveFrame({ CDM_C.VIEWERS.BUFF }, function(frame)
            ...
            if catID and catID ~= false and Shared.HasEquivalentSpellID(groupedSet, catID) then
                ...
                displayID = Shared.GetFrameDisplaySpellID(frame)
            end
    ...
    local leftScroll = CreateFrame("ScrollFrame", "MidnightCDM_BuffGroupsLeftScroll", page, "ScrollFrameTemplate")
    leftScroll:SetPoint("TOPLEFT", LEFT_INSET - SCROLL_LEFT_PAD, -56)
    leftScroll:SetPoint("BOTTOMLEFT", LEFT_INSET - SCROLL_LEFT_PAD, 26)
    leftScroll:SetWidth(LEFT_WIDTH + SCROLL_LEFT_PAD)
    if UI.EnableClipping then UI.EnableClipping(leftScroll) end
    if UI.StyleScrollFrame then UI.StyleScrollFrame(leftScroll) end
    ...
    rightPanel:SetPoint("BOTTOMRIGHT", -16, 26)
    if UI.EnableClipping then UI.EnableClipping(rightPanel) end

    local rightPlaceholder = CreateFrame("Frame", nil, rightPanel)
    rightPlaceholder:SetAllPoints()

    local placeholderTitle = rightPlaceholder:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
    placeholderTitle:SetPoint("TOP", 0, -20)
    placeholderTitle:SetJustifyH("CENTER")
    placeholderTitle:SetText(L["Select a group or spell to edit settings"])
    UI.SetTextMuted(placeholderTitle)

    local signalLine = rightPlaceholder:CreateTexture(nil, "ARTWORK")
    signalLine:SetTexture(CDM_C.TEX_WHITE8X8 or "Interface\\Buttons\\WHITE8X8")
    signalLine:SetSize(3, 46)
    signalLine:SetPoint("TOPLEFT", 34, -58)
    signalLine:SetVertexColor(1, 0.72, 0.12, 0.95)

    local signalTitle = rightPlaceholder:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
    signalTitle:SetPoint("TOPLEFT", signalLine, "TOPRIGHT", 10, 2)
    signalTitle:SetText("|cffffc44d!  " .. L["Blizzard load required"] .. "|r")

    local signalText = rightPlaceholder:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font12")
    signalText:SetPoint("TOPLEFT", signalTitle, "BOTTOMLEFT", 0, -5)
    signalText:SetPoint("TOPRIGHT", rightPlaceholder, "TOPRIGHT", -34, -80)
    signalText:SetJustifyH("LEFT")
    signalText:SetWordWrap(true)
    signalText:SetText(L["Missing buffs must be enabled in Blizzard Cooldown Settings under Tracked Buffs for this spec. If a buff is under Not Displayed, MCDM cannot add it."])
    signalText:SetTextColor(1, 0.78, 0.32, 1)

    local rightPlaceholderSettingsBtn = UI.CreateActionButton(rightPlaceholder, L["Open Settings"], 124, 22)
    rightPlaceholderSettingsBtn:SetPoint("TOPLEFT", signalText, "BOTTOMLEFT", 0, -14)
    rightPlaceholderSettingsBtn:SetScript("OnClick", OpenBlizzardCooldownSettings)
    ...
    local function GetViewerSpellListForSpec(specID)
        if specID == playerSpecID then
            local seen, list = {}, {}
            Shared.ForEachCooldownViewerInfoByKind("buff", function(cdID, info, _, entry)
                if not seen[cdID] then
                    seen[cdID] = true
                    local sid = Shared.GetCooldownDisplaySpellID(info)
                    if sid then
                        list[#list + 1] = {
                            cdID = cdID,
                            spellID = sid,
                            ...
                            displaySpellID = entry and entry.displaySpellID,
                            linkedSpellIDs = (entry and entry.linkedSpellIDs) or info.linkedSpellIDs,
                        }
                    end
                end
            end)
            ...
            for _, entry in ipairs(raw) do
                ...
                if sid and cdID and not entry.isInvisible and not seen[cdID] then
                    seen[cdID] = true
                    list[#list + 1] = {
                        cdID = cdID,
                        spellID = sid,
    ...
    local function MarkLoadedBuffSpellID(set, spellID)
        if type(set) ~= "table" or not IsSafeNumber(spellID) or spellID <= 0 then return end
        set[spellID] = true

        local baseID = NormalizeToBase and NormalizeToBase(spellID)
        if IsSafeNumber(baseID) and baseID > 0 then
            set[baseID] = true
        end

        local stableID = API.ResolveStableBase and API:ResolveStableBase(spellID)
        if IsSafeNumber(stableID) and stableID > 0 then
            set[stableID] = true
        end

        if API.ForEachSpellMatchCandidate then
            API:ForEachSpellMatchCandidate(spellID, function(candidateID)
                if IsSafeNumber(candidateID) and candidateID > 0 then
    ...
    local function IsLoadedBuffSpellID(set, spellID)
        if type(set) ~= "table" or not IsSafeNumber(spellID) or spellID <= 0 then
            return false
        end
        if API.IsCustomBuffTemplateSpell and API:IsCustomBuffTemplateSpell(spellID) then
            return true
        end
        if set[spellID] then return true end

        local baseID = NormalizeToBase and NormalizeToBase(spellID)
        if IsSafeNumber(baseID) and set[baseID] then return true end

        local stableID = API.ResolveStableBase and API:ResolveStableBase(spellID)
        if IsSafeNumber(stableID) and set[stableID] then return true end
    ...
    local function BuildLoadedBuffSpellSetForSpec(specID)
        local set = {}
        if not specID then
            return set, false
        end
        local cacheMissing = specID ~= playerSpecID and not API:GetSpecBuffSpellCache(specID)
        local list = GetViewerSpellListForSpec(specID)
        for _, entry in ipairs(list or {}) do
            MarkLoadedBuffSpellID(set, entry.spellID)
            MarkLoadedBuffSpellID(set, entry.baseSpellID)
            MarkLoadedBuffSpellID(set, entry.displaySpellID)
            MarkLoadedBuffSpellID(set, entry.overrideSpellID)
            MarkLoadedBuffSpellID(set, entry.overrideTooltipSpellID)
            if type(entry.linkedSpellIDs) == "table" then
                for _, linkedID in ipairs(entry.linkedSpellIDs) do
                    MarkLoadedBuffSpellID(set, linkedID)
                end
            end
        end
        return set, cacheMissing
    end
    ...
    local function GetUntrackedViewerSpellListForCurrentSpec()
        ...
        API:ForEachActiveFrame({ CDM_C.VIEWERS.BUFF }, function(frame)
            ...
            if not IsSafeNumber(activeID) then
                activeID = Shared.GetFrameDisplaySpellID(frame)
            end
        ...
        local seen, list = {}, {}
        Shared.ForEachCooldownViewerInfoByKind("buff", function(cdID, info)
            if not seen[cdID] then
                seen[cdID] = true
                local sid = Shared.GetCooldownDisplaySpellID(info)
    ...
    local function GetUngroupedBuffSpellsFromCache(specID)
        ...
        for _, entry in ipairs(rawCache) do
            ...
            if spellID and not entry.isInvisible
                and not Shared.HasEquivalentSpellID(assigned, spellID)
    ...
    local function ShowGroupSettings(groupIndex)
        ...
        Shared.RenderGroupSettingsPanel({
            ...
            statePrefix = "buff-group-settings",
    ...
    local function BuildOverrideSection(rc, yOff, spellID, groupIndex, existingOv, ensureOv, defaults, placeholderOpts, isCustomBuff)
        yOff = yOff - 10
        local overrideHeader = rc:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font18")
        ...
        if soundChecked then
            ...
            if soundOnShowEnabled then
                local showDropdown = RegisterRightPanelDropdown(UI.CreateDropdown(rc, 220, ov.soundOnShow or "None"))
            ...
            if soundOnHideEnabled then
                local hideDropdown = RegisterRightPanelDropdown(UI.CreateDropdown(rc, 220, ov.soundOnHide or "None"))
        ...
        if ttsChecked then
            ...
            local voiceBtn = UI.CreateActionButton(rc, L["Voice Settings"], 120, 22)
            ...
            if ttsOnShowEnabled then
                local ttsShowBox = UI.CreateModernEditBox(rc, 140, 22)
                ...
                local ttsShowHint = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font12")
            ...
            if ttsOnHideEnabled then
                local ttsHideBox = UI.CreateModernEditBox(rc, 140, 22)
                ...
                local ttsHideHint = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font12")
        ...
        if not isCustomBuff then
            yOff = Shared.BuildTextOverrideWidgets(rc, yOff, {
                ...
                createDropdown = function(p) return RegisterRightPanelDropdown(UI.CreateDropdown(p)) end,
            })
        end
    ...
    ShowSpellSettings = function(spellID, groupIndex)
        ...
        local yOff = 0
        local previewGroups = groupIndex and GetSpecGroups() or nil
        local previewGroup = previewGroups and previewGroups[groupIndex]
        local previewOv = previewGroup and Shared.GetMergedOverrideEntry(previewGroup.spellOverrides, spellID)
            or (not groupIndex and GetUngroupedOverride(spellID))
        local previewUseTextOv = previewOv and previewOv.textOverride
        local previewCdSize = (previewUseTextOv and previewOv.cooldownFontSize)
            or (previewGroup and previewGroup.cooldownFontSize)
            or (CDM.db and CDM.db.buffCooldownFontSize)
            or 12
        local previewCdColor = (previewUseTextOv and previewOv.cooldownColor)
            or (previewGroup and previewGroup.cooldownColor)
            or (CDM.db and CDM.db.buffCooldownColor)
        local previewCountSize = (previewUseTextOv and previewOv.countFontSize)
            or (previewGroup and previewGroup.countFontSize)
            or (CDM.db and CDM.db.countFontSize)
            or 15
        local previewCountColor = (previewUseTextOv and previewOv.countColor)
            or (previewGroup and previewGroup.countColor)
            or (CDM.db and CDM.db.countColor)
        local previewCountPos = (previewUseTextOv and previewOv.countPosition)
            or (previewGroup and previewGroup.countPosition)
            or (CDM.db and CDM.db.countPositionMain)
            or "BOTTOMRIGHT"
        local previewCountX = (previewUseTextOv and previewOv.countOffsetX)
            or (previewGroup and previewGroup.countOffsetX)
            or (CDM.db and CDM.db.countOffsetXMain)
            or 0
        local previewCountY = (previewUseTextOv and previewOv.countOffsetY)
            or (previewGroup and previewGroup.countOffsetY)
            or (CDM.db and CDM.db.countOffsetYMain)
            or 0
        local previewActiveSet = BuildActiveSpellSet()
        local previewActive = IsSpellInActiveSet(previewActiveSet, spellID)
            or Shared.HasEquivalentSpellID(previewActiveSet, spellID)
        if not previewActive and NormalizeToBase then
            local previewBaseID = NormalizeToBase(spellID)
            previewActive = Shared.HasEquivalentSpellID(previewActiveSet, previewBaseID)
        end
        local previewGlowEnabled = API:GetSpellGlowEnabled(currentSpecID, spellID)
        local previewGlowColor = API:GetSpellGlowColor(currentSpecID, spellID)
        local previewW = (previewGroup and previewGroup.iconWidth) or 36
        local previewH = (previewGroup and previewGroup.iconHeight) or 36
        local cbEntry = GetCustomBuffEntry(displaySpellID)

        local iconContainer = CreateFrame("Frame", nil, rc)
        iconContainer:SetSize(previewW, previewH)
        ...
        Shared.ApplyDummyIconPreview(iconContainer, {
            width = previewW,
            height = previewH,
            iconTextureRegion = iconTex,
            displaySpellID = displaySpellID,
            texture = tex,
            inactive = not previewActive,
            glowEnabled = previewGlowEnabled,
            glowColor = previewGlowColor,
            borderColor = existingColor,
            cooldownFontSize = previewCdSize,
            cooldownColor = previewCdColor,
            secondaryFontSize = previewCountSize,
            secondaryColor = previewCountColor,
            secondaryPosition = previewCountPos,
            secondaryOffsetX = previewCountX,
            secondaryOffsetY = previewCountY,
            secondaryText = "2",
        })

        local spellName = rc:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font18")
        ...
        yOff = yOff - math.max(40, previewH + 12)

        local borderLabel = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
        ...
        local glowColorLabel = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
        ...
        if isCustom then
            ...
            if not (cbEntry and cbEntry.triggerType) then
                ...
                local sidLabel = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
                sidLabel:SetPoint("TOPLEFT", 0, yOff)
                sidLabel:SetText(L["Spell ID:"])

                local sidInput = UI.CreateModernEditBox(rc, 100, 20)
                ...
                local durLabel = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
                durLabel:SetPoint("TOPLEFT", 0, yOff)
                durLabel:SetText(L["Duration (sec):"])

                local durInput = UI.CreateModernEditBox(rc, 60, 20)
                ...
                local saveBtn = UI.CreateActionButton(rc, L["Save"], 80, 22, "primary")
                saveBtn:SetPoint("TOPLEFT", 0, yOff)

                local cbStatusText = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font12")
                ...
                saveBtn:SetScript("OnClick", function()
                    ...
                    if API.ReplaceCustomBuffSpell then
                        local ok, errCode = API:ReplaceCustomBuffSpell(spellID, newSID, newDur, nil, currentSpecID)
                        if not ok then
                            local msg = L["Failed - invalid spell ID"]
                            if errCode == "custom_buff_exists" then
                                msg = L["Custom buff already exists"]
                            elseif errCode == "custom_buff_not_found" then
                                msg = L["Custom buff not found"]
                            elseif errCode == "invalid_spell_id" then
                                msg = L["Invalid spell ID"]
                            elseif errCode == "not_loaded_in_cdm" or errCode == "cache_missing" or errCode == "cdm_unavailable" then
                                msg = L["This spell is not loaded in CDM for this spec."]
                            end
                            cbStatusText:SetText("|cffff4444" .. msg .. "|r")
                            return
                        end
                    elseif cbEntry then
                        cbEntry.duration = newDur
                    end
    ...
    local ungroupedCacheMessage = leftChild:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
    ...
    ShowSpellPickerPanel = function(groupIndex)
        ...
        Shared.RenderSpellPicker({
            ...
            helpText = L["Missing buffs must be enabled in Blizzard Cooldown Settings under Tracked Buffs for this spec. If a buff is under Not Displayed, MCDM cannot add it."],
            helpButtonText = L["Open Settings"],
            onHelpButton = OpenBlizzardCooldownSettings,
            spells = spells,
            ...
            emptyText = currentSpecID == playerSpecID
                and (L["No loaded buff icons are available. Open Blizzard Settings and move missing buffs from Not Displayed to Tracked Buffs."])
                or (L["All loaded buff icons are assigned to groups. Missing buffs must be enabled in Blizzard Settings first."]),
    ...
    ShowCustomBuffAddPanel = function(targetGroupIndex)
        ...
        HideWorldPreview()
        local _, rc = CreateRightScrollContent(620)
        local yOff = 0
        local loadedBuffSet, isCDMLoadCacheMissing = BuildLoadedBuffSpellSetForSpec(currentSpecID)
        ...
        local ruleText = rc:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font12")
        ruleText:SetPoint("TOPLEFT", 0, yOff)
        ruleText:SetPoint("TOPRIGHT", rc, "TOPRIGHT", -8, yOff)
        ruleText:SetJustifyH("LEFT")
        ruleText:SetWordWrap(true)

        local ruleMessage = L["Quick Add uses built-in MCDM templates. Custom Spell IDs must be loaded by Blizzard Cooldown Manager for this spec."]
        if isCDMLoadCacheMissing then
            local _, specName = GetSpecializationInfoByID(currentSpecID)
            ruleMessage = ruleMessage .. "\n" .. string.format(L["Log %s to build the CDM buff list first."], specName or L["this spec"])
        else
            ruleMessage = ruleMessage .. "\n" .. L["If a manual spell is blocked, Blizzard CDM has not loaded it for this spec."]
            ruleMessage = ruleMessage .. "\n" .. L["Open Blizzard Settings and move the buff from Not Displayed to Tracked Buffs first."]
        end
        ruleText:SetText(ruleMessage)
        UI.SetTextMuted(ruleText)
        yOff = yOff - (ruleText:GetStringHeight() + 14)

        local openSettingsBtn = UI.CreateActionButton(rc, L["Open Settings"], 124, 22)
        openSettingsBtn:SetPoint("TOPLEFT", 0, yOff)
        openSettingsBtn:SetScript("OnClick", OpenBlizzardCooldownSettings)
        yOff = yOff - 34

        local templates = CDM.CustomBuffTemplates or {}
        if #templates > 0 then
            local quickLabel = rc:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
            ...
            for _, tmpl in ipairs(templates) do
                ...
                local isLoaded = IsLoadedBuffSpellID(loadedBuffSet, sid)
                ...
                local tName = tRow:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font12")
                tName:SetPoint("LEFT", tIcon, "RIGHT", 6, 0)

                local tAddBtn = UI.CreateActionButton(tRow, L["Add"], 50, 20)
                tAddBtn:SetPoint("RIGHT", -4, 0)
                tName:SetPoint("RIGHT", tAddBtn, "LEFT", -8, 0)
                tName:SetJustifyH("LEFT")

                local suffix = "  |cff888888" .. dur .. "s|r"
                if alreadyExists then
                    suffix = suffix .. "  |cff888888" .. L["Already added"] .. "|r"
                elseif not isLoaded then
                    suffix = suffix .. "  |cffffaa44" .. L["Not loaded in CDM for this spec"] .. "|r"
                end
                tName:SetText((spellName or tostring(sid)) .. suffix)

                tAddBtn:SetEnabled(not alreadyExists and isLoaded)
                tAddBtn:SetScript("OnClick", function()
                    ...
                    local ok = API:AddCustomBuffSpell(sid, dur, ov, currentSpecID)
        ...
        yOff = yOff - 10
        local advLabel = rc:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
        advLabel:SetPoint("TOPLEFT", 0, yOff)
        advLabel:SetText(L["Custom Spell"])
        UI.SetTextWhite(advLabel)
        yOff = yOff - 24

        local sidLabel = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font12")
        sidLabel:SetPoint("TOPLEFT", 0, yOff)
        sidLabel:SetText(L["Spell ID:"])
        ...
        local durLabel = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font12")
        durLabel:SetPoint("TOPLEFT", 0, yOff)
        durLabel:SetText(L["Duration (sec):"])
        ...
        local previewText = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font12")
        previewText:SetPoint("TOPLEFT", sidInput, "TOPRIGHT", 8, -3)
        previewText:SetWidth(260)
        previewText:SetJustifyH("LEFT")
        previewText:SetText("")

        local advAddBtn = UI.CreateActionButton(rc, L["Add Spell"], 100, 22, "primary")
        advAddBtn:SetPoint("TOPLEFT", 0, yOff)

        local statusText = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font12")
        statusText:SetPoint("LEFT", advAddBtn, "RIGHT", 8, 0)
        statusText:SetText("")

        local function GetManualCustomBuffValidation()
            local sid = tonumber(sidInput:GetText())
            if not sid or sid <= 0 then
                return sid, nil, false, false, ""
            end

            local info = C_Spell.GetSpellInfo(sid)
            if not info then
                return sid, nil, false, false, "|cffff4444" .. L["Invalid spell ID"] .. "|r"
            end

            local name = info.name or tostring(sid)
            if not IsLoadedBuffSpellID(loadedBuffSet, sid) then
                return sid, info, true, false, "|cffffaa44" .. name .. " - " .. L["Not loaded in CDM for this spec"] .. "|r"
            end

            return sid, info, true, true, "|cff00ff00" .. name .. " - " .. L["Loaded in CDM"] .. "|r"
        end

        local function RefreshManualCustomBuffValidation()
            local _, _, isValidSpell, isLoaded, preview = GetManualCustomBuffValidation()
            local dur = tonumber(durInput:GetText())
            previewText:SetText(preview or "")
            advAddBtn:SetEnabled(isValidSpell and isLoaded and dur and dur > 0)
            statusText:SetText("")
        end

        sidInput:SetScript("OnTextChanged", RefreshManualCustomBuffValidation)
        durInput:SetScript("OnTextChanged", RefreshManualCustomBuffValidation)

        advAddBtn:SetScript("OnClick", function()
            local sid, _, isValidSpell, isLoaded = GetManualCustomBuffValidation()
            local dur = tonumber(durInput:GetText())
            ...
            if not isLoaded then
                statusText:SetText("|cffffaa44" .. (L["This spell is not loaded in CDM for this spec."]) .. "|r")
                return
            end
            ...
            local ok, errCode = API:AddCustomBuffSpell(sid, dur, nil, currentSpecID)
            if not ok then
                local msg = L["Failed - invalid spell ID"]
                if errCode == "not_loaded_in_cdm" or errCode == "cache_missing" or errCode == "cdm_unavailable" then
                    msg = L["This spell is not loaded in CDM for this spec."]
                end
        ...
        RefreshManualCustomBuffValidation()
        yOff = yOff - 30

        yOff = yOff - 10
        local disclaimer = rc:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
        ...
        local backBtn = UI.CreateActionButton(rc, L["Back"], 80, 22)
    ...
    local function ConfigureSpellRow(widget, parent, spellID, sourceGroup, y, isActive, spellIndex, spellCount, tooltipOverrides)
        ...
        local hasMoveButtons = sourceGroup and spellIndex and spellCount
        ...
        Shared.LayoutGroupEditorRow(widget, hasMoveButtons and true or false)
    ...
    local function BuildLeftPanel()
        ...
        if isViewingPlayer then
            tooltipOverrideMap = {}
            Shared.ForEachCooldownViewerInfoByKind("buff", function(_, info)
                Shared.MarkTooltipOverride(tooltipOverrideMap, info)
            end)
        end
        ...
        if not btnRefs.group then
            local addGroupBtn = UI.CreateActionButton(page, L["Add Group"], 90, 22)
        ...
        if not btnRefs.icon then
            local addIconBtn = UI.CreateActionButton(page, L["Add Icon"], 90, 22)
        ...
        if not btnRefs.customBuff then
            local addCustomBuffBtn = UI.CreateActionButton(page, L["Add Custom Buff"], 140, 22, "primary")
        ...
        if isViewingPlayer then
            ...
            for displayIdx, item in ipairs(mergedList) do
                if item.isCustom then
                    ...
                    Shared.LayoutGroupEditorRow(widget, true)
            ...
            for groupIndex, groupData in ipairs(groups) do
                ...
                h.deleteBtn:SetScript("OnClick", function()
                    ...
                    if spellCount > 0 then
                        local dialog = StaticPopupDialogs["MidnightCooldownManager_CONFIRM_DELETE_GROUP"]
                        dialog.text = string.format(
                            L["Delete group with %d spell(s)?"],
                            spellCount
                        )
                        dialog._pendingDelete = DoDelete
                        StaticPopup_Show("MidnightCooldownManager_CONFIRM_DELETE_GROUP")
                    else
                        DoDelete()
                    end
                end)
