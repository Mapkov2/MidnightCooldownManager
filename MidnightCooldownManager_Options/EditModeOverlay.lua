...
local VIEWER_CATALOG = CDM.GetCooldownViewerEditModeViewerCatalog and CDM:GetCooldownViewerEditModeViewerCatalog() or {}
local VIEWER_LABELS = VIEWER_CATALOG.labels or {}
local VIEWER_ORDER = VIEWER_CATALOG.order or {}
local VIEWER_HEADER_LETTERS = VIEWER_CATALOG.headerLetters or {}
...
local VIEWER_COUNT = #VIEWER_ORDER
local ICON_CLUSTER_WIDTH = VIEWER_COUNT > 0 and (ICON_SIZE * VIEWER_COUNT + ICON_GAP * (VIEWER_COUNT - 1)) or ICON_SIZE
...
local function GetIndicatorState(policyState, systemIndex)
    local systemIndexSet = policyState and policyState.systemIndexSet
    if not systemIndexSet or not systemIndexSet[systemIndex] then
        return "na"
    end
...
local function CreatePolicyRow(parent, contentWidth)
    ...
    local label = row:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
...
local function CreateEditModeOverlay()
    ...
    for i, letter in ipairs(VIEWER_HEADER_LETTERS) do
        local fs = headerCluster:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
        fs:SetText(letter)
    ...
    local presetBanner = window:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
    ...
    local applyBtn = UI.CreateActionButton(window, L["Apply"], 120, 24, "primary")
    ...
    local cancelBtn = UI.CreateActionButton(window, L["Cancel"], 120, 24)
