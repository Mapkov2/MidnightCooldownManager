...
local function CreateBlacklistOverlay()
    ...
    local titleText = window:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
    ...
    addRow:SetSize(contentWidth, 26)
    ...
    local statusText = window:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
...
local function CreateTrinketsTab(page, tabId)
    local scrollChild, scrollFrame = UI.CreateScrollableTab(page, "MidnightCDM_TrinketsScrollFrame", 700, 500)

    local sections = {}
    local essentialOnlyControls = {}
    local layoutSection, iconSizeSection, positionSection, cooldownSection
    ...
    local function AddSection(title, key, height, defaultOpen)
        local section, body = UI.CreateAccordionSection(scrollChild, title, 540, height, "trinkets:" .. key, defaultOpen, Relayout)
    ...
    page.controls.trinketsEnabled:SetPoint("TOPLEFT", 0, 0)

    local manageBlacklistBtn = UI.CreateActionButton(scrollChild, L["Manage Blacklist"], 150, 24)
    ...
    local layoutBody
    layoutSection, layoutBody = AddSection(L["Layout Mode"], "layout-mode", 220, true)

    local lblMode = layoutBody:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
    lblMode:SetText(L["Display Mode"])
    lblMode:SetPoint("TOPLEFT", 0, 0)

    local ddMode = UI.CreateDropdown(layoutBody)
    ddMode:SetPoint("TOPLEFT", lblMode, "BOTTOMLEFT", 0, -10)
    ddMode:SetWidth(200)
    ddMode:SetDefaultText(GetModeLabel(GetCurrentMode()))
    page.controls.trinketsMode = ddMode
    local lblRow = layoutBody:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
    lblRow:SetText(L["Row"])
    lblRow:SetPoint("TOPLEFT", ddMode, "BOTTOMLEFT", 0, -15)
    table.insert(essentialOnlyControls, lblRow)

    local ddRow = UI.CreateDropdown(layoutBody)
    ...
    local iconSizeBody
    iconSizeSection, iconSizeBody = AddSection(L["Icon Size"], "icon-size", 130, true)
    ...
    page.controls.trinketsIconWidthSlider:SetPoint("TOPLEFT", 0, 0)
    ...
    page.controls.trinketsIconHeightSlider:SetPoint("TOPLEFT", page.controls.trinketsIconWidthSlider, "BOTTOMLEFT", 0, -10)
    ...
    page.controls.trinketsOffsetYSlider:SetPoint("TOPLEFT", page.controls.trinketsOffsetXSlider, "BOTTOMLEFT", 0, -10)

    local cooldownBody
    cooldownSection, cooldownBody = AddSection(L["Cooldown"], "cooldown", 70, true)
    ...
    page.controls.trinketsCooldownFontSizeSlider:SetPoint("TOPLEFT", 0, 0)
    ...
    local function UpdateSectionVisibility()
        ...
        layoutSection:SetContentHeight(isEssential and 250 or 105)
        iconSizeSection:SetShown(not isEssential)
        positionSection:SetShown(isIndependent)
        cooldownSection:SetShown(not isEssential)
        Relayout()
    end
