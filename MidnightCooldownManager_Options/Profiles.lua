...
local function SetupDefaultProfileDropdown(dropdown)
    dropdown:SetDefaultText(MidnightCooldownManagerDB.global.defaultProfile or "Default")
    dropdown:SetupMenu(function(_, rootDescription)
        local profiles = API:GetProfileList()
        local currentDefault = MidnightCooldownManagerDB.global.defaultProfile or "Default"
        for _, name in ipairs(profiles) do
            rootDescription:CreateRadio(name, function()
                return currentDefault == name
            end, function()
                MidnightCooldownManagerDB.global.defaultProfile = name
...
local function RefreshProfilesUI()
    ...
    if copyDropdown then
        SetupProfileDropdown(copyDropdown, true, function(name)
            StaticPopup_Show("MidnightCooldownManager_CONFIRM_COPY_PROFILE", name, nil, { name = name })
        end)
    end

    if deleteDropdown then
        SetupProfileDropdown(deleteDropdown, true, function(name)
            StaticPopup_Show("MidnightCooldownManager_CONFIRM_DELETE_PROFILE", name, nil, { name = name })
        end)
    end
...
local function CreateProfilesTab(page, tabId)
    ...
    local content, scrollFrame = UI.CreateScrollableTab(page, "MidnightCDM_ProfilesScrollFrame", 620, 760)
    ...
    h2:SetPoint("TOPLEFT", 0, 0)

    switchDropdown = UI.CreateDropdown(page, 200, API:GetActiveProfileName())
    ...
    local newEditBox = UI.CreateModernEditBox(newProfileRow, 200, 28)
    newEditBox:SetPoint("LEFT", 0, 0)
    newEditBox:SetMaxLetters(50)

    local createBtn = UI.CreateActionButton(newProfileRow, L["Create"], 80, 26, "primary")
    createBtn:SetPoint("LEFT", newEditBox, "RIGHT", 8, 0)

    local createStatus = newProfileRow:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
    ...
    local copyDesc = page:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
    ...
    copyDropdown = UI.CreateDropdown(page, 200, L["Select Source..."])
    ...
    renameEditBox = UI.CreateModernEditBox(renameRow, 200, 28)
    renameEditBox:SetPoint("LEFT", 0, 0)
    renameEditBox:SetMaxLetters(50)
    renameEditBox:SetText(API:GetActiveProfileName())

    local renameBtn = UI.CreateActionButton(renameRow, L["Rename"], 80, 26)
    renameBtn:SetPoint("LEFT", renameEditBox, "RIGHT", 8, 0)

    local renameStatus = renameRow:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
    ...
    local resetBtn = UI.CreateActionButton(manageRow, L["Reset Profile"], 120, 26, "danger")
    resetBtn:SetPoint("LEFT", 0, 0)
    resetBtn:SetScript("OnClick", function()
        StaticPopup_Show("MidnightCooldownManager_CONFIRM_RESET_PROFILE")
    end)

    deleteDropdown = UI.CreateDropdown(manageRow, 160, L["Delete Profile..."])
    ...
    defaultDropdown = UI.CreateDropdown(page, 200)
    ...
    for i = 1, numSpecs do
        ...
        local label = specSection:CreateFontString(nil, "ARTWORK", "MidnightCDM_Font14")
        label:SetPoint("TOPLEFT", col, 0)
        label:SetText(specName or string.format(L["Spec %d"], i))
        UI.SetTextMuted(label)

        local dd = UI.CreateDropdown(specSection, 140)
    ...
    if UI.FinalizeScroll and scrollFrameChild then
        UI.FinalizeScroll(scrollFrameChild, page, -560)
    end
end
