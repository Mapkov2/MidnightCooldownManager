...
StaticPopupDialogs["MidnightCooldownManager_CONFIRM_DELETE_CD_GROUP"] = {
    text = "",
    button1 = YES,
    button2 = NO,
    OnAccept = function()
        local fn = StaticPopupDialogs["MidnightCooldownManager_CONFIRM_DELETE_CD_GROUP"]._pendingDelete
        if fn then fn() end
    end,
    timeout = 0,
    whileDead = true,
...
local function CreateCooldownGroupsPanel(subPage, page)
    ...
    local tex = UI.CreateDivider(subPage)
    ...
    local function BuildCooldownActiveSet()
        ...
        API:ForEachActiveFrame({ CDM_C.VIEWERS.ESSENTIAL, CDM_C.VIEWERS.UTILITY }, function(frame)
            Shared.MarkFrameSpellCandidates(active, frame)
        end)
        return active
    end
    ...
    local gridEmptyText = iconGridFrame:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
    ...
    local leftScroll = CreateFrame("ScrollFrame", "MidnightCDM_CDGroups_LeftScroll", subPage, "ScrollFrameTemplate")
    leftScroll:SetPoint("TOPLEFT", buttonRow, "BOTTOMLEFT", -SCROLL_LEFT_PAD, -4)
    leftScroll:SetPoint("BOTTOMLEFT", subPage, "BOTTOMLEFT", LEFT_INSET - SCROLL_LEFT_PAD, 26)
    ...
    signalText:SetText(L["Missing cooldown icons must be enabled in Blizzard Cooldown Settings for this spec. If an icon is under Not Displayed, MCDM cannot add it."])
    ...
    ShowSpellSettings = function(spellID, groupIndex)
        ...
        if groupIndex then
            ...
            previewGroup = grps and grps[groupIndex]
            previewOv = previewGroup and Shared.GetMergedOverrideEntry(previewGroup.spellOverrides, spellID)
        else
            previewOv = GetUngroupedOverride(spellID)
        end

        local previewActiveSet = BuildCooldownActiveSet()
        local resolvedPreviewID = ResolveCooldownOverrideID(spellID)
        local previewActive = Shared.HasEquivalentSpellID(previewActiveSet, spellID)
            or Shared.HasEquivalentSpellID(previewActiveSet, resolvedPreviewID)
            or Shared.HasEquivalentSpellID(previewActiveSet, displayID)
        ...
        local previewCdSize = (previewUseTextOv and previewOv.cooldownFontSize)
            ...
            or (CDM.db and CDM.db.cooldownFontSize)
            or 12
        local previewCdColor = (previewUseTextOv and previewOv.cooldownColor)
            or (previewGroup and previewGroup.cooldownColor)
            or (CDM.db and CDM.db.cooldownColor)
        local previewChargeSize = (previewUseTextOv and previewOv.chargeFontSize)
            or (previewGroup and previewGroup.chargeFontSize)
            or (CDM.db and CDM.db.chargeFontSize)
            or 15
        local previewChargeColor = (previewUseTextOv and previewOv.chargeColor)
            or (previewGroup and previewGroup.chargeColor)
            or (CDM.db and CDM.db.chargeColor)
        local previewChargePos = (previewUseTextOv and previewOv.chargePosition)
            or (previewGroup and previewGroup.chargePosition)
            or (CDM.db and CDM.db.chargePosition)
            or "BOTTOMRIGHT"
        local previewChargeX = (previewUseTextOv and previewOv.chargeOffsetX)
            or (previewGroup and previewGroup.chargeOffsetX)
            or (CDM.db and CDM.db.chargeOffsetX)
            or 0
        local previewChargeY = (previewUseTextOv and previewOv.chargeOffsetY)
            or (previewGroup and previewGroup.chargeOffsetY)
            or (CDM.db and CDM.db.chargeOffsetY)
            or 0
        local previewIsDotDefault = DOT_OVERRIDE_SPELLS and DOT_OVERRIDE_SPELLS[spellID]
        local previewShowAura = previewOv and previewOv.showAuraOverlay
        if previewShowAura == nil then
            previewShowAura = previewIsDotDefault or false
        end
        local previewGlowEnabled = (previewShowAura and previewOv and previewOv.auraGlowEnabled)
            or (previewOv and previewOv.readyGlowEnabled)
        local previewGlowColor = (previewShowAura and previewOv and previewOv.auraGlowColor)
            or (previewOv and previewOv.readyGlowColor)
        local previewBorderColor = previewShowAura and previewOv and previewOv.auraBorderEnabled and previewOv.auraBorderColor or nil
        local previewW = (previewGroup and previewGroup.iconWidth) or 40
        local previewH = (previewGroup and previewGroup.iconHeight) or 40
        ...
        Shared.ApplyDummyIconPreview(iconContainer, {
            ...
            secondaryFontSize = previewChargeSize,
            secondaryColor = previewChargeColor,
            secondaryPosition = previewChargePos,
            secondaryOffsetX = previewChargeX,
            secondaryOffsetY = previewChargeY,
            secondaryText = "2",
        })
        yOff = yOff - math.max(54, previewH + 14)
        ...
        do
            ...
            if showAura then
                ...
                local agcLabel = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
                ...
                local abcLabel = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
            ...
            if readyGlowEnabled then
                local rgcLabel = rc:CreateFontString(nil, "OVERLAY", "MidnightCDM_Font14")
    ...
    local function GetViewerSpellListForSpec(specID)
        ...
        if specID == playerSpecID then
            Shared.ForEachCooldownViewerInfoByKind("active", function(cdID, info)
    ...
    local function GetUngroupedSpellsFromViewers()
        ...
        API:ForEachActiveFrame({ CDM_C.VIEWERS.ESSENTIAL, CDM_C.VIEWERS.UTILITY }, function(frame, vName)
            local displayID, record = Shared.GetFrameDisplaySpellID(frame)
            if not (frame:IsShown() or record) then return end
            local slotKey = (record and record.cooldownID) or displayID
    ...
    ShowSpellPickerPanel = function(groupIndex)
        ...
        Shared.RenderSpellPicker({
            ...
            helpText = L["Missing cooldown icons must be enabled in Blizzard Cooldown Settings for this spec. If an icon is under Not Displayed, MCDM cannot add it."],
            ...
            emptyText = currentSpecID == playerSpecID
                and (L["No loaded cooldown icons are available. Open Blizzard Settings and move missing icons out of Not Displayed."])
                or (L["All loaded cooldown icons are assigned to groups. Missing icons must be enabled in Blizzard Settings first."]),
            onSelect = function(sid, cdID)
                ...
                if cdID then
                    local info = Shared.GetCooldownInfoByID(cdID)
            ...
            for groupIndex, groupData in ipairs(groups) do
                ...
                h.deleteBtn:SetScript("OnClick", function()
                    ...
                    if spellCount > 0 then
                        local dialog = StaticPopupDialogs["MidnightCooldownManager_CONFIRM_DELETE_CD_GROUP"]
                        dialog.text = string.format(
                            L["Delete group with %d spell(s)?"],
                            spellCount
                        )
                        dialog._pendingDelete = DoDelete
                        StaticPopup_Show("MidnightCooldownManager_CONFIRM_DELETE_CD_GROUP")
                    else
                        DoDelete()
                    end
                end)
    ...
    do
        local addGroupBtn = UI.CreateActionButton(buttonRow, L["Add Group"], 90, 22)
        ...
        local addIconBtn = UI.CreateActionButton(buttonRow, L["Add Icon"], 90, 22)
