...
local function CountActiveItemFrames(systemFrame)
    return CDM.CountCooldownViewerActiveFrames and CDM:CountCooldownViewerActiveFrames(systemFrame) or 0
end
...
function CDM:GetCooldownViewerLockMessage(systemFrame)
    ...
    return L["Edit Mode locked - use /mcdm"]
end
...
function CDM:SetCooldownViewerLockText(systemFrame, shown)
    ...
    if vName == VIEWERS.ESSENTIAL or vName == VIEWERS.UTILITY then
        ...
        text:SetText(L["Edit Mode locked - use /mcdm"])
    end
...
local function ShowLockNotice()
    if not CDM.editModeCooldownViewerNoticeShown then
        print("|cffffd200MidnightCooldownManager:|r " .. L["Cooldown Viewer settings are managed by /mcdm."])
...
function CDM:UpdateEditModeSelectionOverlay(vName)
    ...
    local viewer = self:GetCooldownViewerFrame(vName)
...
function CDM:LockCooldownViewerEditModeFrames()
    for _, name in ipairs(LOCK_FRAME_NAMES) do
        local frame = self:GetCooldownViewerFrame(name)
...
local function BuildEditModeViewerCatalog()
    ...
    local specs = {
        { systemIndex = SI.Essential, labelKey = "Essential", headerLetter = "E" },
        { systemIndex = SI.Utility, labelKey = "Utility", headerLetter = "U" },
        { systemIndex = SI.BuffIcon, labelKey = "Buff Icons", headerLetter = "B" },
        { systemIndex = SI.BuffBar, labelKey = "Buff Bars", headerLetter = "Bb" },
    }

    for _, spec in ipairs(specs) do
        local systemIndex = spec.systemIndex
        if type(systemIndex) == "number" then
            catalog.order[#catalog.order + 1] = systemIndex
            catalog.labels[systemIndex] = spec.labelKey
            catalog.headerLetters[#catalog.headerLetters + 1] = spec.headerLetter
        end
    end
...
function CDM:GetCooldownViewerEditModeViewerCatalog()
    if not EDIT_MODE_VIEWER_CATALOG then
        EDIT_MODE_VIEWER_CATALOG = BuildEditModeViewerCatalog()
    end
    return EDIT_MODE_VIEWER_CATALOG
end
