...
local GetFrameItemID = CDM.GetFrameItemID
...
local function SafeNumber(value, fallback)
    if value == nil or type(value) ~= "number" then return fallback end
...
local cdFont = _G["MidnightCDM_CDFont"] or CreateFont("MidnightCDM_CDFont")
local cdFontBuff = _G["MidnightCDM_CDFont_Buff"] or CreateFont("MidnightCDM_CDFont_Buff")
...
local function RefreshStyleCache()
    ...
    styleCache.cooldownIconTintEnabled = CfgValue(db, defaults, "cooldownIconTintEnabled", false) == true
    styleCache.buffIconTintEnabled = CfgValue(db, defaults, "buffIconTintEnabled", false) == true
    styleCache.cooldownIconTintColor = CfgValue(db, defaults, "cooldownIconTintColor", { r = 0, g = 0, b = 0, a = 1 })
...
local function GetEffectiveCooldownSpellID(frame)
    ...
    local id = record.displaySpellID or record.overrideTooltipSpellID or record.overrideSpellID or record.spellID
...
local function GetCastSpellID(frame)
    ...
    local record = CDM.GetFrameCooldownRecord and CDM:GetFrameCooldownRecord(frame)
    if not record then return nil end
    local id = record.overrideSpellID or record.spellID or record.baseSpellID
    return IsSafeNumber(id) and id or nil
end
CDM.GetCastSpellID = GetCastSpellID

local function HasChargeSource(frame)
    return CDM.FrameHasChargeVisualSource and CDM:FrameHasChargeVisualSource(frame) or false
end

local function FindAuraOverlayEntry(frame)
    local map = CDM._auraOverlayEnabled
    if not map then return nil end
    local cdID = CDM.GetFrameCooldownID and CDM:GetFrameCooldownID(frame)
    if cdID and map[cdID] then return map[cdID] end
    return nil
end

local function DurationObjectHasRemaining(durationObj)
    if not (durationObj and durationObj.EvaluateRemainingDuration) then return false end
    local remaining = durationObj:EvaluateRemainingDuration(DesaturationCurve, RealTime)
    remaining = SafeNumber(remaining)
    return remaining ~= nil and remaining > 0.001
end

local function CooldownTimesActive(startTime, duration)
    startTime = SafeNumber(startTime, 0)
    duration = SafeNumber(duration, 0)
    return startTime > 0 and duration > 0 and (startTime + duration) > GetTime()
end

local function CooldownFrameIsActive(cooldown)
    if not (cooldown and cooldown.GetCooldownTimes) then return false end
    ...
    local now = GetTime()
    if startTime > (now * 10) or duration > 1000 then
        return (startTime + duration) > (now * 1000)
    end
    return (startTime + duration) > now
end

local function ResolveIconTintHost(frame)
    if not frame then return nil, nil end
    local icon = frame.Icon
    if icon and icon.IsObjectType and icon:IsObjectType("Texture") then
        return frame, icon
    end
    if icon and icon.Icon and icon.Icon.IsObjectType and icon.Icon:IsObjectType("Texture") then
        return icon, icon.Icon
    end
    return nil, nil
end

local function SetIconCooldownTint(frame, enabled, active, color)
    local overlay = frame and frame.cdmCooldownTintOverlay
    if not (enabled and active) or frame.cdmVisualsHidden then
        if overlay then overlay:Hide() end
        return
    end

    local host, iconTexture = ResolveIconTintHost(frame)
    if not (host and iconTexture) then return end

    if not overlay or overlay:GetParent() ~= host then
        overlay = host:CreateTexture(nil, "ARTWORK", nil, 2)
        overlay:SetTexture(CDM_C.TEX_WHITE8X8 or "Interface\\Buttons\\WHITE8X8")
        frame.cdmCooldownTintOverlay = overlay
    end

    overlay:ClearAllPoints()
    overlay:SetAllPoints(iconTexture)
    color = color or CDM_C.BLACK or { r = 0, g = 0, b = 0, a = 1 }
    overlay:SetColorTexture(color.r or 0, color.g or 0, color.b or 0, color.a or 1)
    overlay:Show()
end

local function ApplyKindIconCooldownTint(frame, kind, active)
    if not frame then return end
    RefreshStyleCache()
    local enabled = (kind == "buff") and styleCache.buffIconTintEnabled or styleCache.cooldownIconTintEnabled
    SetIconCooldownTint(frame, enabled, active, styleCache.cooldownIconTintColor)
end

function CDM:ApplyIconCooldownTint(frame, kind, active)
    ApplyKindIconCooldownTint(frame, kind, active)
end

function CDM:RefreshBuffIconCooldownTint(frame)
    ApplyKindIconCooldownTint(frame, "buff", CooldownFrameIsActive(frame and frame.Cooldown))
end
...
local function ApplyIconDesat(frame, entry, auraActive, sid, blizzDesat)
    ...
    elseif sid and not HasChargeSource(frame) then
        ...
        tintActive = DurationObjectHasRemaining(realDur)
        ...
        if boolDesat == nil and CDM.GetFrameCooldownDesaturated then
            boolDesat = CDM:GetFrameCooldownDesaturated(frame)
        end
        tintActive = boolDesat == true
    ...
    SetIconCooldownTint(frame, styleCache.cooldownIconTintEnabled, tintActive, styleCache.cooldownIconTintColor)
end
...
local function ApplyCooldownWidget(frame, entry, auraActive, sid)
    ...
    elseif entry and entry.auraOverlay and not auraActive and entry.auraDesaturateInactive then
        ...
        if HasChargeSource(frame) then
            ...
            tintActive = DurationObjectHasRemaining(chargeDur)
            if chargeDur then
            ...
            tintActive = DurationObjectHasRemaining(cdDur)
            if cdDur then
...
function CDM:RefreshFrameVisuals(frame, skipDesat)
    ...
    local auraActive = CDM.IsFrameUsingAuraDisplayTime and CDM:IsFrameUsingAuraDisplayTime(frame) or false
...
local function ApplyBuffCooldownStyle(frame)
    ...
    CDM:RefreshBuffIconCooldownTint(frame)
end
...
local function EnsureFrameHooks(frame, hookType)
    ...
    if hookType == "buff" then
        ...
        if not frame.cdmBuffSwipeHooked then
            ...
            hooksecurefunc(cd, "SetCooldown", function(_, startTime, duration)
                ApplyBaseSwipeStyle(cd, frame)
                SetIconCooldownTint(frame, styleCache.buffIconTintEnabled, CooldownTimesActive(startTime, duration), styleCache.cooldownIconTintColor)
            end)
            hooksecurefunc(cd, "Clear", function()
                SetIconCooldownTint(frame, styleCache.buffIconTintEnabled, false, styleCache.cooldownIconTintColor)
            end)
...
function CDM:ApplyStyle(frame, vName, forceUpdate)
    ...
    if needsVisualUpdate then
        ...
        frame.Cooldown:SetCountdownFont(isBuff and "MidnightCDM_CDFont_Buff" or "MidnightCDM_CDFont")
            ...
            if not frame.cdmCooldownHooked then
                ...
                hooksecurefunc(cd, "SetCooldown", function()
                    ...
                    if entry
                       ...
                       or (CDM.IsFrameUsingAuraDisplayTime and CDM:IsFrameUsingAuraDisplayTime(frame))
                ...
                hooksecurefunc(cd, "Clear", function()
                    if frame.cdmInternalWrite then return end
                    SetIconCooldownTint(frame, styleCache.cooldownIconTintEnabled, false, styleCache.cooldownIconTintColor)
                    local entry = FindAuraOverlayEntry(frame)
...
RefreshKeybindForFrame = function(frame, KB, kbCacheVer, styleVersion)
    ...
    if not kbText and GetFrameItemID then
        local itemID = GetFrameItemID(CDM, frame)
        if itemID then
            kbText = KB:GetKeybindTextForItem(itemID)
        end
    end
    if kbText then
        kbFS:SetText(kbText)
...
CDM:RegisterRefreshCallback("iconCooldownTint", function()
    RefreshStyleCache()
    CDM:ForEachActiveFrame({ VIEWERS.ESSENTIAL, VIEWERS.UTILITY }, function(itemFrame)
        CDM:RefreshFrameVisuals(itemFrame)
    end)
    CDM:ForEachActiveFrame({ VIEWERS.BUFF }, function(itemFrame)
        CDM:RefreshBuffIconCooldownTint(itemFrame)
    end)
    local customBuffs = CDM.CustomBuffs
    if customBuffs and customBuffs.iconFrames then
        for spellID, itemFrame in pairs(customBuffs.iconFrames) do
            CDM:ApplyIconCooldownTint(itemFrame, "buff", customBuffs.activeBuffs and customBuffs.activeBuffs[spellID] ~= nil)
        end
    end
end, 31, { "STYLE" })

function CDM:InstallStyleAcquireResetHook(v)
    CDM:InstallCooldownViewerAcquireCallback(v, "style", function(itemFrame)
        ...
        if itemFrame.cdmCooldownTintOverlay then
            itemFrame.cdmCooldownTintOverlay:Hide()
        end
    end)
end
