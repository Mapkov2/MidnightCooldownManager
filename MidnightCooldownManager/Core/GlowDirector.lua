...
function GlowDirector:OnCooldownIDSet(frame)
    ...
    local cdID = record and record.cooldownID or (CDM.GetFrameCooldownID and CDM:GetFrameCooldownID(frame))
    if not cdID then return end
    local readySet = CDM._readyGlowCooldownIDs
    if not readySet or not readySet[cdID] then return end

    local spellID = record and (record.overrideSpellID or record.spellID or record.baseSpellID)
...
function GlowDirector:InstallAcquireResetHook(v)
    CDM:InstallCooldownViewerAcquireCallback(v, "glowDirector", function(itemFrame)
        itemFrame.cdmGlowDirectorCdID = nil

        if not CDM.InstallCooldownItemLifecycleCallbacks then return end
        CDM:InstallCooldownItemLifecycleCallbacks(itemFrame, OWNER_KEY, {
            onCooldownSet = function(self)
                if CDM.InvalidateFrameCooldownRecord then
                    CDM:InvalidateFrameCooldownRecord(self)
                end
                local cdID = CDM.GetFrameCooldownID and CDM:GetFrameCooldownID(self)
                if cdID == self.cdmGlowDirectorCdID then return end
                GlowDirector:OnCooldownIDSet(self)
            end,

            onCooldownCleared = function(self)
                if CDM.InvalidateFrameCooldownRecord then
                    CDM:InvalidateFrameCooldownRecord(self)
                end
            ...
            onOverrideSpellChanged = function(self)
                if CDM.InvalidateFrameCooldownRecord then
                    CDM:InvalidateFrameCooldownRecord(self)
                end
