...
if LSM then
    LSM:Register("border", "Midnight_Thin", "Interface\\AddOns\\MidnightCooldownManager\\Media\\Borders\\Midnight_Thin.tga")
    LSM:Register("border", "Midnight_Empty", "Interface\\AddOns\\MidnightCooldownManager\\Media\\Borders\\Midnight_Empty.tga")
...
local function GetBorderDef()
    local borderKey = CDM_C.GetConfigValue("borderFile", "1 Pixel")
    local borderFile
    if LSM then
    ...
    if not borderFile and borderKey ~= "None" then
        borderFile = "Interface\\AddOns\\MidnightCooldownManager\\Media\\Borders\\Midnight_Thin.tga"
    end
...
function BORDER:InstallAcquireResetHook(v)
    CDM:InstallCooldownViewerAcquireCallback(v, "border", function(itemFrame)
