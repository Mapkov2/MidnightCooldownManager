...
local function ComputeUtilityVisibleCount()
    ...
    CDM:ForEachActiveFrame({ VIEWERS.UTILITY }, function(frame)
...
local function HasCooldownRecord(frame)
    return GetFrameCooldownRecord and GetFrameCooldownRecord(CDM, frame) ~= nil
end
...
local function CollectAndOrderRecords(icons, isEssential, maxRowEss, injFrames)
    ...
    for _, frame in ipairs(icons) do
        local spellID = ResolveBaseSpellID(frame)
        if spellID or HasCooldownRecord(frame) then
...
function CDM:PositionEssentialOrUtilityIcons(icons, viewer, vName)
    ...
    if not inCombat then
        local viewerFrame = CDM:GetCooldownViewerFrame(vName)
        if viewerFrame then
...
local function CalculateEssentialRow1Width()
    ...
    local activeCount = 0
    CDM:ForEachActiveFrame({ VIEWERS.ESSENTIAL }, function(frame)
        if frame:IsShown() then
            local spellID = ResolveBaseSpellID(frame)
            if spellID then
...
function CDM:InstallLayoutAcquireResetHook(v)
    CDM:InstallCooldownViewerAcquireCallback(v, "layout", function(itemFrame)
