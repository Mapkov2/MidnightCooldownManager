...
local GetFrameCooldownRecord = CDM.GetFrameCooldownRecord
...
local function IsBuffFrameIncluded(frame)
    ...
    if GetFrameCooldownRecord and GetFrameCooldownRecord(CDM, frame) then return true end
    return false
end
...
local function CollectBuffFramesInto(buffTbl, groupTbls, inEditMode)
...
local function CollectFramesForReanchor(activeViewer, activeVName, inEditMode)
    if not activeViewer then return end
    if activeVName ~= VIEWERS.ESSENTIAL and activeVName ~= VIEWERS.UTILITY then return end

    CDM:ForEachActiveFrame({ activeVName }, function(frame)
        if frame:IsShown() or inEditMode or (GetFrameCooldownRecord and GetFrameCooldownRecord(CDM, frame)) then
...
local function RunBuffPipeline(activeSelf, viewer, vName, full)
    ...
    CollectBuffFramesInto(tempBuff, tempBuffGroups, inEditMode)
...
local function CollectCrossViewerGroupFrames(activeVName, inEditMode)
    ...
    CDM:ForEachActiveFrame({ oppositeVName }, function(frame)
        if frame:IsShown() or inEditMode or (GetFrameCooldownRecord and GetFrameCooldownRecord(CDM, frame)) then
            local cdGroupIdx = CheckCdGroupMatch(frame)
            if cdGroupIdx then
                if not tempCdGroups[cdGroupIdx] then
...
local function CollectBarFramesInto(barTbl, groupTbls, inEditMode)
    local viewer = CDM:GetCooldownViewerFrame(VIEWERS.BUFF_BAR)
    if not viewer then return viewer end

    CDM:ForEachActiveFrame({ VIEWERS.BUFF_BAR }, function(frame)
        local matchType, _, groupIdx = CheckBarRegistryMatch(frame)
        if matchType == "bargroup" and groupIdx then
            if not groupTbls[groupIdx] then groupTbls[groupIdx] = {} end
            groupTbls[groupIdx][#groupTbls[groupIdx] + 1] = frame
        elseif inEditMode or frame:IsShown() or (GetFrameCooldownRecord and GetFrameCooldownRecord(CDM, frame)) then
